...
Select name, monthlymaintenance, 
	CASE when monthlymaintenance < 100 THEN 'cheap'
		 ELSE 'expensive' END AS maintenancelabel
from Facilities; 
...
the member name. */
select DISTINCT f.name, CONCAT(m.firstname,' ', m.surname) as membername from Facilities as f
...
select sub.name, SUM(sub.cost) as revenue from 
...
SELECT m.surname, m.firstname , mem.surname as recomm_surname, mem.firstname as recomm_firstname  from Members as m
JOIN Members as mem
ON m.recommendedby = mem.memid
where LENGTH(m.recommendedby) <> 0
ORDER BY m.surname, m.firstname

/* Q12: Find the facilities with their usage by member, but not guests */
SELECT f.name , COUNT(DISTINCT(b.memid)) as Members from Bookings b 
JOIN Facilities f ON b.facid = f.facid
where memid <> 0 -- not guest
group by f.name;

/* Q13: Find the facilities usage by month, but not guests */
SELECT EXTRACT(month from b.starttime) as Month , COUNT(DISTINCT(b.memid)) as Members from Bookings b 
JOIN Facilities f ON b.facid = f.facid
where memid <> 0
group by Month;
